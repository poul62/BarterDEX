...
$('.porfolio_coins_list').on('click', '.btn-portfoliogo', function() {
	$('#dashboard-coins').hide();
	$('#dashboard-exchange').show();
...
function displayPortfolio() {
  ...
  $('.porfolio_coins_list').append(actiavte_portfolio_coins_list_spinner);
	CheckPortfolioFn();
...
$('.btn-bot_action-trigger').click(function(e){
	e.preventDefault();
	var default_lang = JSON.parse(sessionStorage.getItem('mm_default_lang'));
	console.log('btn-botlistrefresh clicked');
	console.log($(this).data());
...
function mk_inv_sendrawtx(mk_inv_rawtx_data,mk_inv_rawtx_coin) {
	...
	$.ajax({
		...
		try {
			...
			bootbox.alert(`Transaction Sent Successfully. Heres the Transaction ID:<br>
				...
				<div class="col-sm-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<h3 class="panel-title">${default_lang.Portfolio.portfolio_add_coins}</h3>
						</div>
...
function PortfolioTblDataFn(portfolio_tbl_data) {
	...
	$.each(portfolio_tbl_data.portfolio, function(index, val) {
		...
		kmdBalance += Number(val.kmd_equiv);
		dex_portfolio_coins_tbl_tr += `
			<li class="coinList-coin ${val.coin} btn-portfoliogo" data-coin="` + val.coin + `" data-coinname="` + coin_name + `" data-addr="` + val.address + `" data-balance="` + val.amount + `">
        <a class="KMD">
           <div class="coinList-coin_icon coin-colorized">
              <i class="coin-icon-svg crypto-icons-pack ${val.coin}"><img src="img/cryptologo/${val.coin.toLowerCase()}.png"></i></div>
           <div class="coinList-coin_balance ${val.coin}">
              <strong class="coinList-coin_balance-name">${val.coin}<!--Komodo--></strong>
              <strong class="coinList-coin_balance-amount">
                 ${val.amount} ${val.coin}
              </strong>
              <!--small>Native mode</small-->
           </div>
           <span class="coinList-coin_action">
		...
		$('.porfolio_coins_list').append(dex_portfolio_coins_tbl_tr);
	})
	$('#kmd-equiv-balance').html(kmdBalance.toFixed(8));
	const _addCoin = `
		<li class="coinList-coin add-new-coin" onClick="addcoins_dialog()">
        <a>
           <div class="coinList-coin_icon coin-colorized"><i class="crypto-icons-pack"></i></div>
           <div class="coinList-coin_balance"><strong>Add new coin</strong></div>
           <span class="coinList-coin_action">
              <svg id="Layer_1" version="1.1" viewBox="0 0 512 512">
                 <polygon points="160,115.4 180.7,96 352,256 180.7,416 160,396.7 310.5,256 "></polygon>
              </svg>
	...
	$('.porfolio_coins_list').append(_addCoin);
};
...
function CheckOrderBookFn(sig) {
	...
	$.ajax({
		...
		if (!data.userpass === false) {
			...
			$('.orderbook_numasks').html((data.numasks ? data.numasks : 0) + ' sellers');
			$('.orderbook_numbids').html((data.numbids ? data.numbids : 0) + ' buyers');

			$('.orderbook_bids tbody').empty();
			$('.orderbook-bids .rt-tbody').empty();
			$.each(data.bids, function(index, val) {
				...
				var orderbook_bids_tr2 = `
          <div class="rt-tr-group ${mytrade_true}">
          	<div class="rt-tr -odd">
            	<div class="rt-td" style="flex: 100 0 auto; width: 100px;" onclick=setOrderPrice(${JSON.stringify(row_trade_data)})>${val.price}</div>
            	<div class="rt-td" style="flex: 100 0 auto; width: 100px;" onclick=setOrderPrice(${JSON.stringify(row_trade_data)})>${((val.avevolume == 0) ? '-' : val.avevolume)}</div>
            	<div class="rt-td" style="flex: 100 0 auto; width: 100px;" onclick=setOrderPrice(${JSON.stringify(row_trade_data)})>${val.depth}</div>
            	<div class="rt-td" style="flex: 100 0 auto; width: 100px;" onclick=setOrderPrice(${JSON.stringify(row_trade_data)})>${val.age}</div>
            	<div class="rt-td" style="flex: 100 0 auto; width: 100px;" onclick=setOrderPrice(${JSON.stringify(row_trade_data)})>${val.numutxos}</div>
            	<div class="rt-td" style="flex: 100 0 auto; width: 100px;" onclick=setOrderPrice(${JSON.stringify(row_trade_data)})><span class="glyphicon glyphicon-piggy-bank" aria-hidden="true"></span> ${val.zcredits.toFixed(2)}</div>
            </div>
          </div>
        `;

				/*orderbook_bids_tr2 += '<tr ' + mytrade_true + '>';
				...
				console.warn(orderbook_bids_tr2);
				$('.orderbook-bids .rt-tbody').append(orderbook_bids_tr2);
			})

			$('.orderbook_asks tbody').empty();
			$('.orderbook-asks .rt-tbody').empty();
			if (data.asks &&
					data.asks.length) {
        ...
        `;
				...
				$('.orderbook-asks .rt-tbody').append(orderbook_asks_tr2);
			})
		}
		  ...
		  backdrop: true,
			message: `<div style="text-align: center; margin-top: -40px;">
					...
					<div style="text-align: center;">
						...
						<pre style="font-size: 18px; color: #000">${data.coin.smartaddress}</pre class="receive_addr_qrcode_addr">
					</div>`
		});
			...
			if (!data.error === false && data.error === 'coin is disabled') {
				...
				if (data.coin.hasOwnProperty('electrum')) {
					...
					if (data.coin.coin == 'KMD') {
						var show_zcredits = '';//'<button class="btn btn-xs btn-warning coin_balance_zcredits"><span class="glyphicon glyphicon-piggy-bank" aria-hidden="true"></span> ' + data.coin.zcredits + '</button>';
					} else {
						var show_zcredits = '';
					}
					$('.trading_sellcoin_balance').html(data.coin.balance + ' <span>' + coin + '</span>' + show_zcredits + '<br><span class="address">' + data.coin.smartaddress + '</span> <!--<button class="btn btn-xs copysellcoinaddr"><i class="fa fa-clipboard" aria-hidden="true"></i></button>-->');
				}
				$('#balance-spinner').hide();
				$('.balance-block').show();
			}
		...
		if (!data.userpass === false) {
			...
			if (!data.error === false && data.error === 'coin is disabled') {
				...
				if (data.coin.hasOwnProperty('electrum')) {
					...
					$('.trading_coin_balance').html(data.coin.balance + ' <span>' + coin + '</span>' + show_zcredits + '<br><span class="address">' + data.coin.smartaddress + '</span> <!--<button class="btn btn-xs btn-copycoinaddr"><i class="fa fa-clipboard" aria-hidden="true"></i></button>-->');
				}
			}
...
function electrum_coin_balance(coin_balance_data) {
	...
	$.ajax({
		...
		if (coin_balance_data.coin == 'KMD') {
			var show_zcredits = '<button class="btn btn-xs btn-warning coin_balance_zcredits"><span class="glyphicon glyphicon-piggy-bank" aria-hidden="true"></span> ' + coin_balance_output_data.zcredits + '</button>';
		} else {
			var show_zcredits = '';
		}

		if(coin_balance_data.baserel == 'base'){
			$('.trading_coin_balance').html(coin_balance_output_data.balance + ' <span>' + coin_balance_data.coin + '</span>' + show_zcredits + '<br><span class="address">' + coin_balance_data.smartaddress + '</span>');
		}
		if(coin_balance_data.baserel == 'rel'){
			$('.trading_sellcoin_balance').html(coin_balance_output_data.balance + ' <span>' + coin_balance_data.coin + '</span>' + show_zcredits + '<br><span class="address">' + coin_balance_data.smartaddress + '</span>');
		}
	}).fail(function(jqXHR, textStatus, errorThrown) {
	    // If fail
	    console.log(textStatus + ': ' + errorThrown);
			...
			$.each(reversed_swap_list, function(index, val) {
				...
				if(!val.error === false) {
					...
					if (val.bob && val.alice) {
						exchange_swap_status_tr2 += `
	           <li class="orders-item">
	              <div class="orders-item-wrapper">
	                 <div class="orders-item-details">
	                    <div class="orders-item-details-coins">
	                       <section class="orders-item-details-coin">
	                          <span class="orders-item-details-coin-amount">${val.alice}</span>
	                          <div class="orders-item-details-coin-icon coin-colorized"><i class="coin-icon-svg"><img src="img/cryptologo/${val.alice.toLowerCase}.png"></i></div>
	                       </section>
	                       <div class="orders-item-details-type">
	                          <i class="orders-item-details-coins-tradeType">
	                       ...
	                       <section class="orders-item-details-coin">
	                          <div class="orders-item-details-coin-icon coin-colorized"><i class="coin-icon-svg"><img src="img/cryptologo/${val.bob.toLowerCase()}.png"></i></div>
	                          <span class="orders-item-details-coin-amount">${val.bob}</span>
	                       </section>
	                       <section class="orders-item-details-price">
	                          <span class="orders-item-details-type-label">
	                            ${val.status}
	                          </span>
	                       </section>
	                    </div>
	           ...
	           <li class="orders-item">
	              <div class="orders-item-wrapper">
	                 <div class="orders-item-details">
	                    <div class="orders-item-details-coins">
	                       ...
	                       <!--section class="orders-item-details-price">
	                          <span class="orders-item-details-type-label">
	                            <button class="btn btn-default swapstatus_details" data-quoteid="${val.quoteid}" data-requestid="${val.requestid}">Details</button>
	                          </span>
	                       </section-->
	                    </div>
	                 </div>
				...
				const _counter = `<div class="myswaps-counter">${swapsCounter} swaps</div>`;
				$('#myswaps').html(exchange_swap_status_tr2 + _counter);
			})
		}
	}).fail(function(jqXHR, textStatus, errorThrown) {
	    // If fail
...
function enableDetailelViewControls(type) {
	if ($('#' + type + '-controls').hasClass('detailed-view')) {
		$('#' + type + '-controls').removeClass('detailed-view');
		$('#' + type + '-controls .wallet-wallets-list-item_action').removeClass('fa-chevron-up').addClass('fa-chevron-down');
	} else {
		$('#' + type + '-controls').addClass('detailed-view');
		$('#' + type + '-controls .wallet-wallets-list-item_action').removeClass('fa-chevron-down').addClass('fa-chevron-up');
	}
}